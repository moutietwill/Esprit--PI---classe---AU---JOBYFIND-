...
class AdminController extends Controller {
    ...
    public function __construct() {
        ...
        if (!isset($_SESSION['user_id']) || !in_array($_SESSION['role'] ?? '', ['Admin', 'Tutor', 'Mentor'])) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                $event = new Event([
                    ...
                    'image' => $this->uploadEventImage($_FILES['image'] ?? null) ?? 'public/assets/images/event/default.jpg',
                ]);
    ...
    public function updateEvent($id) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                $newImage = $this->uploadEventImage($_FILES['image'] ?? null);
                if ($newImage) {
    ...
    /**
     * Display formations management page
     */
    public function formations() {
        $formations = $this->fetchAllFormations();
        $this->render('admin/formations', ['formations' => $formations]);
    }
    ...
    public function storeFormation() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                $result = $this->persistFormation($formation);
                
                if ($result) {
                    $this->redirect('/admin/formations?success=1&action=create');
                } else {
                    $this->redirect('/admin/formations?error=1');
                }
            } catch (Exception $e) {
                error_log('Error creating formation: ' . $e->getMessage());
                $this->redirect('/admin/formations?error=1&msg=' . urlencode($e->getMessage()));
            }
        }
    }
    ...
    public function updateFormation($id) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                if (!$formation) {
                    $this->redirect('/admin/formations?error=1&msg=Formation not found');
                    return;
                }

                $formation->setTitre($_POST['titre'] ?? '');
                $formation->setPrix((float)($_POST['prix'] ?? 0));
                $formation->setDate($_POST['date'] ?? '');
                $formation->setDuree($_POST['duree'] ?? '');
                $formation->setDescription($_POST['description'] ?? '');
                $formation->setCategorie($_POST['categorie'] ?? '');

                $result = $this->persistFormation($formation);
                
                if ($result) {
                    $this->redirect('/admin/formations?success=1&action=update');
                } else {
                    $this->redirect('/admin/formations?error=1');
                }
            } catch (Exception $e) {
    ...
    public function deleteFormation($id) {
        try {
            $result = $this->removeFormationRecord($id);
            
            if ($result) {
                $this->redirect('/admin/formations?success=1&action=delete');
            } else {
                $this->redirect('/admin/formations?error=1');
            }
        } catch (Exception $e) {
