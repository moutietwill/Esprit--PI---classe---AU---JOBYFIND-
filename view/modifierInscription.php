...
$inscription = $inscriptionC->getInscriptionById($id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (
        ...
        $inscriptionC->updateInscription(
            $id,
            $_POST['nom'],
            $_POST['prenom'],
            $_POST['email'],
...
<title>Jobyfind – Modifier l'inscription</title>
...
<style>
  ...
  .payment-option { flex: 1; position: relative; }
  .payment-option input[type="radio"] { position: absolute; opacity: 0; width: 0; height: 0; }
  .payment-option label {
    display: flex; flex-direction: column; align-items: center; gap: 8px;
    padding: 18px 14px; border: 2px solid var(--gray-200); border-radius: 12px;
    cursor: pointer; transition: all .25s ease; background: var(--gray-50);
    font-size: .82rem !important; font-weight: 600 !important; color: var(--gray-600) !important;
  }
  .payment-option label:hover { border-color: var(--blue); background: #EFF6FF; }
  .payment-option input:checked + label {
    border-color: var(--blue); background: #EFF6FF; color: var(--blue) !important;
    box-shadow: 0 0 0 3px rgba(37,99,235,.10);
  }
  .payment-icon { font-size: 2rem; line-height: 1; }
  .payment-check { display: none; position: absolute; top: 8px; right: 10px; color: var(--blue); font-size: .9rem; font-weight: 800; }
  .payment-option input:checked ~ .payment-check { display: block; }
</style>
</head>
...
<div class="back-modal" style="width:100%; max-width:580px; position:relative; animation:slideUp .25s ease; box-shadow:0 10px 40px rgba(0,0,0,0.2);">
  <form action="" method="POST" id="formModifierInscription" novalidate onsubmit="return validateModifierInscription(event)">
    <div class="back-modal-header" style="padding:24px 28px 18px; border-bottom:1px solid var(--gray-100); display:flex; align-items:center; justify-content:space-between; position:sticky; top:0; background:#fff; z-index:1;">
      <div>
        ...
        <h3 style="font-size:1.15rem; color:var(--navy); margin:0; font-family:'Nunito',sans-serif; font-weight:800;">Modifier l'inscription</h3>
      </div>
      <a href="backofficeInscription.php" class="modal-close" style="text-decoration:none; background:var(--gray-100); width:32px; height:32px; border-radius:50%; display:flex; align-items:center; justify-content:center; color:var(--gray-500); font-size:1.1rem;">✕</a>
    </div>
    ...
    <div class="back-modal-body" style="padding:24px 28px; max-height:65vh; overflow-y:auto;">
        ...
        <div class="field-group">
          ...
          <select id="id_formation" name="id_formation">
            ...
            <?php foreach ($listeFormations as $f): 
              $sel = ($inscription['id_formation'] == $f['id']) ? 'selected' : '';
            ?>
              <option value="<?= $f['id'] ?>" <?= $sel ?>><?= htmlspecialchars($f['titre']) ?> (<?= $f['prix'] ?> €)</option>
            <?php endforeach; ?>
          </select>
      ...
      <div class="row-2">
        <div class="field-group">
          <label>Nom *</label>
          <input type="text" id="nom" name="nom" placeholder="Ex: Dupont" value="<?= htmlspecialchars($inscription['nom']) ?>">
          <span class="error-msg" id="err-nom">Le nom est obligatoire et doit contenir uniquement des lettres.</span>
        </div>
        <div class="field-group">
          <label>Prénom *</label>
          <input type="text" id="prenom" name="prenom" placeholder="Ex: Jean" value="<?= htmlspecialchars($inscription['prenom']) ?>">
      ...
      <div class="row-2">
        <div class="field-group">
          <label>Email *</label>
          <input type="text" id="email" name="email" placeholder="Ex: jean@email.com" value="<?= htmlspecialchars($inscription['email']) ?>">
          <span class="error-msg" id="err-email">L'email doit contenir "@" et "." (ex: nom@domaine.com).</span>
        </div>
        <div class="field-group">
          <label>Numéro de téléphone *</label>
          <input type="text" id="telephone" name="telephone" placeholder="Ex: 12345678" value="<?= htmlspecialchars($inscription['telephone']) ?>">
        ...
        <div class="field-group">
          ...
          <div class="payment-options">
            <div class="payment-option">
              <input type="radio" name="methode_paiement" id="paiement_d17" value="D17" <?= ($inscription['methode_paiement'] === 'D17') ? 'checked' : '' ?>>
            ...
            <div class="payment-option">
              <input type="radio" name="methode_paiement" id="paiement_visa" value="Visa Card" <?= ($inscription['methode_paiement'] === 'Visa Card') ? 'checked' : '' ?>>
    ...
    <div class="back-modal-footer" style="padding:16px 28px 24px; display:flex; gap:10px; justify-content:flex-end; border-top:1px solid var(--gray-100);">
      <a href="backofficeInscription.php" class="btn-cancel" style="text-decoration:none; padding:10px 22px; border:1.5px solid var(--gray-200); border-radius:var(--radius-sm); font-size:.88rem; font-weight:600; color:var(--gray-600); background:var(--white); display:flex; align-items:center;">Annuler</a>
...
<script>
function validateModifierInscription(e) {
  var valid = true;
