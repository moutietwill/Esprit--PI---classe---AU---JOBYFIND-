...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (
        ...
        if ($_POST["methode_paiement"] === "Visa Card") {
            ...
            $apiKey = api_stripe::getSecretKey();
            if (empty($apiKey)) {
                file_put_contents(__DIR__ . '/stripe_debug.log', "[" . date('Y-m-d H:i:s') . "] API Key is EMPTY\n", FILE_APPEND);
            } else {
                $maskedKey = substr($apiKey, 0, 8) . "..." . substr($apiKey, -4);
                file_put_contents(__DIR__ . '/stripe_debug.log', "[" . date('Y-m-d H:i:s') . "] Using API Key: $maskedKey (Length: " . strlen($apiKey) . ")\n", FILE_APPEND);
            }
            ...
            if ($curl_error) {
                file_put_contents(__DIR__ . '/stripe_debug.log', "[" . date('Y-m-d H:i:s') . "] CURL ERROR: $curl_error\n", FILE_APPEND);
                $error = "Erreur de connexion (CURL): " . $curl_error;
            } else {
                ...
                if ($http_code === 200 && isset($result['url'])) {
                    ...
                    $errorMsg = $result['error']['message'] ?? "Erreur inconnue (HTTP $http_code)";
                    file_put_contents(__DIR__ . '/stripe_debug.log', "[" . date('Y-m-d H:i:s') . "] STRIPE ERROR: $errorMsg\n", FILE_APPEND);
                    $error = "Erreur de communication avec Stripe: " . $errorMsg;
                }
            }
            ...
            FormationMailer::sendConfirmationEmail($_POST['email'], $_POST['nom'], $_POST['prenom'], $selected_formation['titre']);
