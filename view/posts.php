...
if (\['REQUEST_METHOD'] === 'POST') {
    if (isset(\['action'])) {
        \ = isset(\['cover_image']) ? \['cover_image'] : null;
    ...
    <?php if (\ === 'add' || \ === 'edit'): ?>
        <div class="table-section">
            <div class="table-header">
                <?= \ === 'edit' ? "Modifier la Formation" : "Créer une Formation" ?>
            ...
            <div style="padding: 24px;">
                <form method="POST" enctype="multipart/form-data" id="formationForm">
                    ...
                    <?php if (\ === 'edit'): ?>
                        ...
                        <input type="hidden" name="old_cover_image" value="<?= \['cover_image'] ?>">
                    <?php endif; ?>

                    <div class="form-grid">
                        <div class="form-group full">
                            <label>Titre de la formation *</label>
                            <input type="text" name="title" id="titleInput" value="<?= \ ? htmlspecialchars(\['title']) : '' ?>">
                        ...
                        <div class="form-group full">
                            ...
                            <textarea name="content" id="contentInput"><?= \ ? htmlspecialchars(\['content']) : '' ?></textarea>
                        ...
                        <div class="form-group">
                            ...
                            <?php if(\ && \['cover_image']): ?>
                                <br><small>Image actuelle: <?= htmlspecialchars(\['cover_image']) ?></small>
                            <?php endif; ?>
                        </div>
                    </div>
    ...
    <?php else: ?>
        <div class="table-section">
            ...
            <table>
                ...
                <tbody>
                    <?php if (empty(\)): ?>
                        <tr><td colspan="4" style="text-align:center;">Aucune formation trouvée.</td></tr>
                    <?php else: ?>
                        <?php foreach(\ as \): ?>
                        <tr>
                            <td style="display: flex; align-items: center; gap: 12px; border-bottom: none; padding-top: 10px; padding-bottom: 10px;">
                                <?php if (isset(\['cover_image']) && \['cover_image']): ?>
                                    <img src="../uploads/<?= htmlspecialchars(\['cover_image']) ?>" style="width: 48px; height: 48px; border-radius: 6px; object-fit: cover; flex-shrink: 0;">
                                <?php else: ?>
                                ...
                                <?php endif; ?>
                                <div>
                                    <strong style="color:var(--text-main); display:block;"><?= htmlspecialchars(\['title']) ?></strong>
                                    <small style="color:var(--text-muted);"><?= htmlspecialchars(\['instructor']) ?></small>
                                </div>
                            </td>
                            <td><?= htmlspecialchars(\['category']) ?></td>
                            <td><?= \['price'] > 0 ? htmlspecialchars(\['price']) . ' TND' : 'Gratuit' ?></td>
                            <td>
                                <span class="status-badge <?= \['status'] == 'draft' ? 'status-draft' : '' ?>">
                                    <?= \['status'] == 'published' ? 'PUBLIÉ' : 'BROUILLON' ?>
                                </span>
                            </td>
                            <td class="actions">
                                <a href="posts.php?action=edit&id=<?= \['id'] ?>" class="action-btn btn-edit"><i class="fas fa-edit"></i></a>
