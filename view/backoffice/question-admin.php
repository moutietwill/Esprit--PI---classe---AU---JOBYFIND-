                ...
                foreach ($_POST['options'] as $index => $texte) {
                    if (!empty(trim($texte))) {
                        ...
                        if ($id_reponse > 0 && in_array($id_reponse, $existingAnsIds)) {
                            ...
                            if ($new_id) $submittedAnsIds[] = $new_id;
                        }
                    }
                }
            } elseif ($_POST['type'] === 'Vrai/Faux' && isset($_POST['vf_correct'])) {
...
if ($result) $listQuestions = $result->fetchAll(PDO::FETCH_ASSOC);

// Récupérer tous les quiz pour le select
$allQuizzes = [];
$qResult = $quizCtrl->listQuiz();
if ($qResult) $allQuizzes = $qResult->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_GET['edit'])) {
    ...
    if ($selectedQuestion) {
        ...
        usort($selectedAnswers, function($a, $b) { return $a['id_reponse'] <=> $b['id_reponse']; });
    }
}
?>
<!DOCTYPE html>
...
<div class="sidebar">
    ...
    <div class="sidebar-section">
        ...
        <a href="question-admin.php" class="sidebar-link active"><i class="fas fa-question-circle"></i> Questions</a>
...
<div class="main">
    ...
    <div class="table-card">
        ...
        <table>
            <thead>
                <tr><th>Énoncé</th><th>Type</th><th>Points</th><th>Quiz</th><th>Date</th><th>Actions</th></tr>
            </thead>
            <tbody>
                <?php if (empty($listQuestions)): ?>
                    <tr><td colspan="6" class="empty-state">Aucune question créée pour le moment.</td></tr>
                <?php else: ?>
                    <?php foreach ($listQuestions as $q): ?>
                    <tr>
                        <td><?= htmlspecialchars(substr($q['enonce'], 0, 60)) ?><?= strlen($q['enonce']) > 60 ? '...' : '' ?></td>
                        ...
                        <td>
                            <div class="action-btns">
                                <a href="?edit=<?= $q['id_question'] ?>" class="action-btn edit" title="Modifier" style="text-decoration:none"><i class="fas fa-edit"></i></a>
                                <button class="action-btn del" onclick="openDeleteModal(<?= $q['id_question'] ?>, '<?= htmlspecialchars(substr($q['enonce'], 0, 30), ENT_QUOTES) ?>')" title="Supprimer">
        ...
        <form method="POST" id="questionForm" onsubmit="return validateForm(event)">
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <textarea name="enonce" id="enonce" class="form-input" rows="3" placeholder="Saisissez la question..."></textarea>
                ...
                <div id="qcm-options" style="display:none; padding:15px; background:#f8fafc; border-radius:10px; margin-top:15px; border:1px solid #e2e8f0;">
                    ...
                    <button type="button" class="btn-outline-sm btn-add-option" onclick="addOptionRow()"><i class="fas fa-plus"></i> Ajouter une option</button>
                </div>

                <!-- DYNAMIC OPTIONS FOR VRAI/FAUX -->
                <div id="vf-options" style="display:none; padding:15px; background:#f8fafc; border-radius:10px; margin-top:15px; border:1px solid #e2e8f0;">
