...
$isTutor = ($role === 'Tutor' || $role === 'Admin' || $role === 'Mentor'); 
...
if (isset($_GET['delete_id']) && $isTutor) {
    ...
    header("Location: quiz_management.php?msg=deleted");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_save']) && $isTutor) {
    ...
    if ($newQuiz->save()) { header("Location: quiz_management.php?msg=success"); exit(); }
}
...
$domainClasses = [
    ...
    'Tech & Dev' => 'tech',
    'RH' => 'rh',
    'Development' => 'tech' // Handling existing data
];
$domainIcons = [
...
<body style="background-color: #f8fafc;">
  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-section">
      ...
      <a class="sidebar-link active" href="quiz_management.php"><i class="fa-solid fa-lightbulb"></i><span>Bibliothèque</span></a>
      <?php if ($isTutor): ?>
  ...
  <div class="modal-overlay" id="quiz-modal">
    <div class="modal">
      ...
      <form method="POST" action="quiz_management.php">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select class="form-input" id="f-domaine" name="domaine" required>
                ...
                <option>Marketing</option><option>Finance</option><option>Tech & Dev</option><option>RH</option>
              </select>
            </div>
            <div class="form-group">
