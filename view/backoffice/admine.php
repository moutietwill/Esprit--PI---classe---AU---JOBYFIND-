...
if (!isset($_SESSION['user_id']) || !isset($_SESSION['role']) || !in_array($_SESSION['role'], ['Admin', 'Tutor', 'Mentor'])) {
...
<head>
  ...
  <style>
    ...
    .quick-admin-links { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 20px; }
    .quick-admin-link { display: inline-flex; align-items: center; gap: 10px; padding: 12px 16px; border-radius: 10px; text-decoration: none; font-weight: 700; font-size: 14px; border: 1px solid #dbe3ef; color: #0f2a5f; background: #fff; box-shadow: 0 1px 2px rgba(15,42,95,.08); }
    .quick-admin-link:hover { border-color: #2d79ff; color: #2d79ff; }
    .quick-admin-link.primary { background: #2d79ff; color: #fff; border-color: #2d79ff; }
    .quick-admin-link.primary:hover { color: #fff; background: #1f63d9; }
  </style>
