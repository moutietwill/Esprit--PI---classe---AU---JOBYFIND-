...
<div class="table-card">
    <div class="table-header">
        <p class="table-title">Candidatures pour l'offre : <strong><?= htmlspecialchars($offre['titre']) ?></strong></p>
        <a href="index.php?action=add_candidature&id_offre=<?= $offre['id_offre'] ?>" class="btn-primary">
            <i class="fa fa-plus"></i> Ajouter une candidature
        </a>
    </div>
    ...
    <div class="search-bar-wrap">
        <div class="search-input-wrap">
            ...
            <input type="text" id="searchBO" placeholder="Rechercher par nom, email, téléphone…" autocomplete="off">
        </div>
    ...
    <table>
        <thead>
            <tr>
                <th><a href="<?= sort_url_bo('id_candidature', $current_sort, $current_order) ?>">ID <?= sort_icon_bo('id_candidature', $current_sort, $current_order) ?></a></th>
                <th><a href="<?= sort_url_bo('nom_candidat', $current_sort, $current_order) ?>">Candidat <?= sort_icon_bo('nom_candidat', $current_sort, $current_order) ?></a></th>
                <th><a href="<?= sort_url_bo('email_candidat', $current_sort, $current_order) ?>">Email <?= sort_icon_bo('email_candidat', $current_sort, $current_order) ?></a></th>
                <th><a href="<?= sort_url_bo('telephone', $current_sort, $current_order) ?>">Téléphone <?= sort_icon_bo('telephone', $current_sort, $current_order) ?></a></th>
                <th><a href="<?= sort_url_bo('date_candidature', $current_sort, $current_order) ?>">Date <?= sort_icon_bo('date_candidature', $current_sort, $current_order) ?></a></th>
                <th><a href="<?= sort_url_bo('statut', $current_sort, $current_order) ?>">Statut <?= sort_icon_bo('statut', $current_sort, $current_order) ?></a></th>
                <th>Actions</th>
            </tr>
        </thead>
        <tbody id="boBody">
            ...
            <?php
                $colors = ['En attente' => 'color:#f59e0b;', 'Acceptée' => 'color:#22c55e;', 'Rejetée' => 'color:#ef4444;'];
                $color  = $colors[$c['statut']] ?? '';
            ?>
            <tr class="bo-row" data-statut="<?= htmlspecialchars($c['statut']) ?>">
                <td><?= htmlspecialchars($c['id_candidature']) ?></td>
                <td><strong><?= htmlspecialchars($c['prenom_candidat'] . ' ' . $c['nom_candidat']) ?></strong></td>
                ...
                <td>
                    <a href="index.php?action=edit_candidature&id=<?= $c['id_candidature'] ?>" class="action-btn edit" title="Modifier"><i class="fa fa-pen"></i></a>
            ...
            <tr><td colspan="7" style="text-align:center;">Aucune candidature pour cette offre</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
    <div id="noResultsBO" style="display:none; text-align:center; padding:20px; color:var(--muted);">
    ...
    <div class="pagination-wrap">
        ...
        <div class="pagination-links">
            <a href="<?= page_url_bo(1) ?>" class="page-btn <?= $page <= 1 ? 'disabled' : '' ?>" title="Première page">
                <i class="fa fa-angles-left"></i>
            </a>
            <a href="<?= page_url_bo($page - 1) ?>" class="page-btn <?= $page <= 1 ? 'disabled' : '' ?>">
            ...
            if ($start > 1): ?>
                <a href="<?= page_url_bo(1) ?>" class="page-btn">1</a>
                <?php if ($start > 2): ?><span class="page-btn" style="border:none;background:none;color:var(--muted);">…</span><?php endif; ?>
            <?php endif; ?>
            <?php for ($i = $start; $i <= $end; $i++): ?>
                <a href="<?= page_url_bo($i) ?>" class="page-btn <?= $i === $page ? 'active' : '' ?>"><?= $i ?></a>
            <?php endfor; ?>
            <?php if ($end < $total_pages): ?>
                <?php if ($end < $total_pages - 1): ?><span class="page-btn" style="border:none;background:none;color:var(--muted);">…</span><?php endif; ?>
                <a href="<?= page_url_bo($total_pages) ?>" class="page-btn"><?= $total_pages ?></a>
            <?php endif; ?>
            <a href="<?= page_url_bo($page + 1) ?>" class="page-btn <?= $page >= $total_pages ? 'disabled' : '' ?>">
                <i class="fa fa-angle-right"></i>
            </a>
            <a href="<?= page_url_bo($total_pages) ?>" class="page-btn <?= $page >= $total_pages ? 'disabled' : '' ?>" title="Dernière page">
...
<div style="margin-top: 20px;">
    <a href="index.php?action=list_offres" class="btn-outline"><i class="fa fa-arrow-left"></i> Retour aux offres</a>
</div>

<script>
(function () {
    const searchInput = document.getElementById('searchBO');
    const rows        = document.querySelectorAll('.bo-row');
    const countEl     = document.getElementById('searchCount');
    const noMsg       = document.getElementById('noResultsBO');
