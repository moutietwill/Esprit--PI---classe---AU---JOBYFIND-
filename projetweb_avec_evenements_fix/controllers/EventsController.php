...
class EventsController extends Controller {
    ...
    public function index() {
        ...
        $counts = [];
        foreach ($events as $e) {
            $counts[$e->getId()] = $this->countInscriptions($e->getId());
        }
        $this->render('events/index', ['events' => $events, 'counts' => $counts]);
    }
    ...
    public function show($id) {
        try {
            ...
            $inscrits = $this->countInscriptions($id);
            $this->render('events/show', ['event' => $event, 'inscrits' => $inscrits]);
        } catch (Exception $e) {
   ...
   public function store() {
    ...
    public function inscrire($idEvenement) {
        ...
        try {
            ...
            // Vérifier la limite de 5 inscrits
            $totalInscriptions = $this->countInscriptions($idEvenement);
            if ($totalInscriptions >= 5) {
                echo json_encode(['success' => false, 'message' => 'Désolé, cet événement est complet (maximum 5 inscriptions).']);
                exit;
            }
        ...
        try {
            ...
            foreach ($events as $ev) {
                $results[] = [
                    'id' => $ev->getId(),
                    'title' => $ev->getTitre(),
                    'description' => mb_substr(strip_tags($ev->getDescription()), 0, 160),
                    'date' => $ev->getDate(),
                    'lieu' => $ev->getLieu(),
                    'image' => $ev->getImage(),
                    'url' => $this->baseUrl('/events/show/' . $ev->getId()),
                ];
            }
