...
class BlogController {
    ...
    function AjouterPost($post, $file = null) {
        ...
        $sql = "INSERT INTO posts (title, excerpt, content, category_id, category, instructor, price, duration_hours, cover_image, status, created_at, updated_at)
                VALUES (:title, :excerpt, :content, :category_id, :category, :instructor, :price, :duration_hours, :cover_image, :status, NOW(), NOW())";
        try {
            $query = $db->prepare($sql);
            $query->execute([
                'title'          => $post->getTitle(),
                ...
                'instructor'     => $post->getInstructor(),
                'price'          => $post->getPrice(),
                'duration_hours' => $post->getDurationHours(),
            ...
            BlogMailer::notifyNewPost($post->getTitle(), $post->getContent(), $categoryName, $post->getStatus());
    ...
    function GetCommentsByPost($postId) {
        $sql = "SELECT id, post_id, user_name, content, created_at FROM (
                    SELECT id, post_id, author_name as user_name, content, created_at FROM comments WHERE post_id = :post_id
                ) as tmp ORDER BY created_at ASC";
        $db = Config::GetConnexion();
        try {
            $query = $db->prepare($sql);
     ...
     */
    public function AddComment($postId, $content, $userName = 'Utilisateur', $userId = 0) {
        $db = Config::GetConnexion();
        // Schema check (non-blocking)
        try {
            $cCols = $db->query("SHOW COLUMNS FROM comments")->fetchAll(PDO::FETCH_COLUMN);
            if (!in_array('user_id', $cCols)) {
                $db->exec("ALTER TABLE comments ADD COLUMN user_id INT(11) NOT NULL DEFAULT 0 AFTER post_id");
            }
        } catch (Exception $e) { /* ignore */ }

        try {
            // Check for author_email column (required by schema)
            $authorEmail = 'guest@example.com'; 

            $sql = "INSERT INTO comments (post_id, user_id, author_name, author_email, content, created_at, status) 
                    VALUES (:post_id, :user_id, :author_name, :author_email, :content, NOW(), 'approved')";
            $query = $db->prepare($sql);
            $query->execute([
                ':post_id' => $postId,
                ':user_id' => $userId,
    ...
    public function HasLiked($postId, $userId) {
        ...
        try {
            if ($userId > 0) {
                ...
                $sql = "SELECT COUNT(*) FROM likes WHERE post_id = :post_id AND user_id = 0 AND user_ip = :user_ip";
                $query = $db->prepare($sql);
    ...
    public function ToggleLike($postId, $userId) {
        ...
        if ($this->HasLiked($postId, $userId)) {
            if ($userId > 0) {
                ...
                $sql = "DELETE FROM likes WHERE post_id = :post_id AND user_id = 0 AND user_ip = :user_ip";
                $query = $db->prepare($sql);
                $query->execute([':post_id' => $postId, ':user_ip' => $userIp]);
            }
            return false; // unliked
        } else {
            $sql = "INSERT INTO likes (post_id, user_id, user_ip) VALUES (:post_id, :user_id, :user_ip)";
            $query = $db->prepare($sql);
            $query->execute([':post_id' => $postId, ':user_id' => $userId, ':user_ip' => $userIp]);
            return true; // liked
        }
    }

    /**
     * S'assure que toutes les tables nécessaires existent (migration automatique).
     */
    public function EnsureSchema() {
        ...
        try {
            ...
            // Fix: Ensure all necessary columns exist if table was already created partially
            $prCols = $db->query("SHOW COLUMNS FROM post_ratings")->fetchAll(PDO::FETCH_COLUMN);
            if (!in_array('user_id', $prCols)) {
                $db->exec("ALTER TABLE post_ratings ADD COLUMN user_id INT(11) NOT NULL DEFAULT 1 AFTER post_id");
            }
            if (!in_array('updated_at', $prCols)) {
                $db->exec("ALTER TABLE post_ratings ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP AFTER created_at");
            }
            
            // Ensure unique constraint exists on (post_id, user_id)
            $indexes = $db->query("SHOW INDEX FROM post_ratings")->fetchAll(PDO::FETCH_COLUMN, 2);
            // Remove old IP-based unique key if it exists
            if (in_array('unique_rating', $indexes)) {
                $db->exec("ALTER TABLE post_ratings DROP INDEX `unique_rating` ");
            }
            // Add new user_id based unique key
            $db->exec("ALTER TABLE post_ratings ADD UNIQUE KEY `unique_rating_user` (`post_id`, `user_id`) ");
            ...
            // Ensure unique constraint exists on (post_id, user_id, user_ip) for likes
            $lIndexes = $db->query("SHOW INDEX FROM likes")->fetchAll(PDO::FETCH_COLUMN, 2);
            if (in_array('unique_like_user', $lIndexes)) {
                $db->exec("ALTER TABLE likes DROP INDEX `unique_like_user` ");
            }
            if (!in_array('unique_like_hybrid', $lIndexes)) {
                $db->exec("ALTER TABLE likes ADD UNIQUE KEY `unique_like_hybrid` (`post_id`, `user_id`, `user_ip`) ");
            }

            // 3. Colonnes manquantes dans posts
            $cols = $db->query("SHOW COLUMNS FROM posts")->fetchAll(PDO::FETCH_COLUMN);
            if (!in_array('rating', $cols)) {
                $db->exec("ALTER TABLE posts ADD COLUMN rating FLOAT DEFAULT 0");
            }
            if (!in_array('reviews_count', $cols)) {
                $db->exec("ALTER TABLE posts ADD COLUMN reviews_count INT DEFAULT 0");
            }
            if (!in_array('views_count', $cols)) {
                $db->exec("ALTER TABLE posts ADD COLUMN views_count INT DEFAULT 0");
            }

            // 4. Table des Commentaires (Ensure Schema)
            $db->exec("
    ...
    public function AddRating($postId, $userId, $rating) {
        ...
        $db = Config::GetConnexion();
        try {
            // INSERT ou UPDATE si le user_id a déjà voté
            ...
            if (!$query->execute([
                ...
                throw new Exception("Échec de l'insertion/mise à jour du rating.");
            }
            ...
            $newAvg   = round((float)($stats['avg_rating'] ?? 0), 1);
            ...
            if (!$updateQuery->execute([
                ...
                throw new Exception("Échec de la mise à jour des stats dans la table posts.");
            }

            return ['avg' => $newAvg, 'count' => $newCount];
        } catch (Exception $e) {
            error_log("AddRating Detailed Error: " . $e->getMessage());
            throw $e; 
        }
    }
    ...
    public function GetUserRating($postId, $userId) {
        ...
        $db = Config::GetConnexion();
        try {
            $sql = "SELECT rating FROM post_ratings WHERE post_id = :post_id AND user_id = :user_id LIMIT 1";
            $query = $db->prepare($sql);
            $query->execute([':post_id' => $postId, ':user_id' => $userId]);
            $row = $query->fetch();
    ...
    /**
     * Retourne la distribution des avis par nombre d'étoiles avec pourcentages.
     * Retourne un tableau: [1 => ['count' => X, 'percentage' => Y], 2 => [...], etc.]
     */
    public function GetRatingDistribution($postId) {
        $db = Config::GetConnexion();
        $distribution = [
        ...
        try {
            $sql = "SELECT rating, COUNT(*) as count FROM post_ratings WHERE post_id = :post_id GROUP BY rating";
            $query = $db->prepare($sql);
            $query->execute([':post_id' => $postId]);
            ...
            foreach ($results as $row) {
                ...
                if (isset($distribution[$rating])) {
                    $distribution[$rating]['count'] = (int)$row['count'];
                    $distribution[$rating]['percentage'] = $totalVotes > 0 ? round(($row['count'] / $totalVotes) * 100, 1) : 0;
                }
            }
