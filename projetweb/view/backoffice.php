...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($page === 'categories') {
        ...
        if ($_POST['action'] === 'add') {
            ...
            BlogMailer::notifyNewPost($_POST['title'], $_POST['content'], $categoryName, $_POST['status']);
        ...
        if ($_POST['action'] === 'add' && !empty($_POST['post_id']) && !empty($_POST['content']) && !empty($_POST['user_name'])) {
            if (!$hasBadWords) {
                ...
                BlogMailer::notifyNewComment($_POST['user_name'], $_POST['content'], $postTitle);
            } else {
...
<body>
  ...
  <aside class="sidebar">
    ...
    <div class="nav-section">
      ...
      <a href="../../public/index.php/admin/events" class="nav-link">
        <i class="fas fa-calendar-days"></i> Événements
      </a>
      <a href="../../public/index.php/quiz" class="nav-link">
    ...
    <div class="nav-section">
      ...
      <a href="javascript:void(0)" onclick="openAIModal()" class="nav-link" style="background:linear-gradient(135deg,rgba(139,92,246,0.18),rgba(29,78,216,0.12));color:#a78bfa;">
  ...
  <main class="main-content">
    ...
    <?php elseif ($page === 'posts'): ?>
      ...
      <?php if ($action === 'add' || $action === 'edit'): ?>
          <!-- Formulaire Manuel (Fixé) -->
          <div class="table-section">
              <div class="table-header">
                  <?= $action === 'edit' ? "Modifier le Blog" : "Créer un Blog" ?>
                  <a href="backoffice.php?page=posts" class="btn btn-light"><i class="fas fa-arrow-left"></i> Retour</a>
              </div>
              <div style="padding: 24px;">
                  <form method="POST" action="backoffice.php?page=posts" enctype="multipart/form-data" id="postForm">
                      ...
                      <?php if ($action === 'edit' && $editPost): ?>
                          ...
                          <input type="hidden" name="old_cover_image" value="<?= htmlspecialchars($editPost['cover_image'] ?? '') ?>">
                      <?php endif; ?>

                      <div class="form-grid">
                          <div class="form-group full">
                              <label>Titre du blog *</label>
                              <input type="text" name="title" id="postTitleInput" value="<?= $editPost ? htmlspecialchars($editPost['title']) : '' ?>">
                              <div id="postTitleError" style="color: #ef4444; font-size: 12px; margin-top: 5px; display: none;"></div>
                          </div>

                          <div class="form-group">
                              <label>Catégorie *</label>
                              <select name="category_id" id="postCategoryInput">
                              ...
                              <div id="postCategoryError" style="color: #ef4444; font-size: 12px; margin-top: 5px; display: none;"></div>
                          </div>

                          <div class="form-group">
                              <label>Auteur *</label>
                              <input type="text" name="instructor" value="<?= $editPost ? htmlspecialchars($editPost['instructor'] ?? '') : '' ?>">
                          </div>
                          ...
                          <div class="form-group full">
                              ...
                              <textarea name="content" id="postContentInput" style="min-height: 200px;"><?= $editPost ? htmlspecialchars($editPost['content']) : '' ?></textarea>
                              <div id="postContentError" style="color: #ef4444; font-size: 12px; margin-top: 5px; display: none;"></div>
                          </div>
                      </div>
  ...
  <!-- Modal IA Générateur -->
  <div id="aiModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.7); z-index: 10000; justify-content: center; align-items: center; backdrop-filter: blur(5px);">
    <div style="background: #0f172a; border-radius: 20px; max-width: 700px; width: 90%; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); overflow: hidden;">
      <div style="padding: 30px; background: linear-gradient(135deg, #1e1b4b 0%, #312e81 100%); position: relative;">
        <button onclick="closeAIModal()" style="position: absolute; right: 20px; top: 20px; background: none; border: none; color: rgba(255,255,255,0.6); font-size: 24px; cursor: pointer;">&times;</button>
        <div style="display: flex; align-items: center; gap: 15px;">
            ...
            <div>
                ...
                <p style="color: rgba(255,255,255,0.6); font-size: 14px; margin: 5px 0 0;">L'IA crée votre article et son image en quelques secondes.</p>
            </div>
        </div>
      </div>
      
      <div style="padding: 35px; background: #0f172a;">
          <div style="margin-bottom: 25px;">
              ...
              <input type="text" id="aiThemeInputModal" placeholder="Ex: Les secrets de l'IA en 2025..." 
                     style="width: 100%; padding: 16px 20px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); color: #fff; font-size: 16px; outline: none; transition: 0.3s;">
          </div>
          ...
          <div style="margin-bottom: 30px;">
              ...
              <select id="aiCategoryInputModal" style="width: 100%; padding: 16px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.1); color: #fff; outline: none; cursor: pointer; font-size: 15px;">
                  <?php foreach($categoriesList as $cat): ?>
          ...
          <button type="button" id="aiGenerateBtnModal" onclick="generateAndPublishModal()" 
                  style="width: 100%; padding: 18px; background: #6366f1; color: #fff; border: none; border-radius: 12px; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 12px; transition: 0.3s; font-size: 15px;">
              <i class="fas fa-magic"></i> GÉNÉRER MAINTENANT
          </button>

          <!-- Status / Success -->
          <div id="aiStatusModal" style="display: none; margin-top: 30px; text-align: center;">
              <div id="aiLoaderModal" style="margin-bottom: 20px;">
                  <div style="display: inline-block; width: 40px; height: 40px; border: 4px solid rgba(255,255,255,0.1); border-top-color: #6366f1; border-radius: 50%; animation: spin 1s linear infinite;"></div>
                  <p style="color: #fff; margin-top: 15px; font-size: 15px;">Inspiration en cours... Votre blog arrive !</p>
              </div>
              <div id="aiResultModal" style="display: none; background: rgba(16,185,129,0.1); border: 1px solid rgba(16,185,129,0.3); border-radius: 15px; padding: 25px; text-align: left;">
                  <div style="display: flex; gap: 20px; align-items: center;">
                      <img id="aiImgPreviewModal" src="" style="width: 150px; height: 100px; border-radius: 10px; object-fit: cover; border: 2px solid rgba(255,255,255,0.1);">
                      <div style="flex: 1;">
                          <h4 style="color: #fff; margin: 0; font-size: 17px;">Blog publié avec succès !</h4>
                          <p id="aiTitleModal" style="color: rgba(255,255,255,0.6); font-size: 14px; margin: 8px 0; font-style: italic;"></p>
                          <a id="aiViewLinkModal" href="#" class="btn" style="background: #6366f1; padding: 8px 16px; font-size: 13px;">Voir l'article</a>
                      </div>
                  </div>
              </div>
          </div>
  ...
  #aiGenerateBtnModal:hover { background: #4f46e5; transform: translateY(-2px); box-shadow: 0 10px 20px rgba(99,102,241,0.2); }
  </style>
  ...
  <script>
    function openAIModal() {
        ...
        document.getElementById('aiStatusModal').style.display = 'none';
        document.getElementById('aiThemeInputModal').value = '';
    }
    ...
    function generateAndPublishModal() {
        const theme = document.getElementById('aiThemeInputModal').value.trim();
        const catId = document.getElementById('aiCategoryInputModal').value;
        const btn = document.getElementById('aiGenerateBtnModal');
        const status = document.getElementById('aiStatusModal');
        const loader = document.getElementById('aiLoaderModal');
        const result = document.getElementById('aiResultModal');
        ...
        fetch('../ajax_generate_blog.php', { method: 'POST', body: formData })
            ...
            .then(data => {
                ...
                if (data.success) {
                    document.getElementById('aiImgPreviewModal').src = data.image;
                    document.getElementById('aiTitleModal').textContent = data.title;
                    result.style.display = 'block';
                    
                    // Auto-reload after 1.5s to show the new post in the table
                    setTimeout(() => {
                        location.href = 'backoffice.php?page=posts';
                    }, 1500);
                } else {
    ...
    // Fermer la modal IA en cliquant en dehors
