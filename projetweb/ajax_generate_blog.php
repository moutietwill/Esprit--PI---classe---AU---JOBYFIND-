...
try {
    ...
    if (!in_array('instructor', $cols)) $db->exec("ALTER TABLE posts ADD COLUMN instructor VARCHAR(255) DEFAULT 'JobyFind AI' AFTER category");
    if (!in_array('price', $cols)) $db->exec("ALTER TABLE posts ADD COLUMN price DECIMAL(10,2) DEFAULT 0.00 AFTER instructor");
    if (!in_array('duration_hours', $cols)) $db->exec("ALTER TABLE posts ADD COLUMN duration_hours INT DEFAULT 0 AFTER price");
    if (!in_array('excerpt', $cols)) $db->exec("ALTER TABLE posts ADD COLUMN excerpt TEXT DEFAULT NULL AFTER title");
} catch (Exception $e) { /* Ignore existing column errors */ }

try {
    $db = Config::GetConnexion();
    $sql = "INSERT INTO posts (title, content, category_id, category, status, cover_image, instructor, created_at, updated_at)
            VALUES (:title, :content, :category_id, :category, :status, :cover_image, :instructor, NOW(), NOW())";
    $stmt = $db->prepare($sql);
    $stmt->execute([
        ':title'       => $finalTitle,
        ':content'     => $fullContent,
