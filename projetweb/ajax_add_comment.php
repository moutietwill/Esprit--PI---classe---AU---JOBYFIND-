...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    session_start();
    // Prioritize session user name if available
    $sessionName = isset($_SESSION['user_name']) ? $_SESSION['user_name'] : (isset($_SESSION['user_email']) ? explode('@', $_SESSION['user_email'])[0] : '');
    $userName = !empty($sessionName) ? $sessionName : (isset($data['user_name']) ? trim($data['user_name']) : (isset($_POST['user_name']) ? trim($_POST['user_name']) : 'Anonyme'));
    ...
    $result = $controller->AddComment($postId, $content, $userName, $userId);
    ...
    if ($result) {
        ...
        // Envoyer la notification par email (Fixed Class Name)
        BlogMailer::notifyNewComment($userName, $content, $postTitle);
